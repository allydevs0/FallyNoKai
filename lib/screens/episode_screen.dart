...
class _EpisodeScreenState extends State<EpisodeScreen> {
  ...
  Future<void> _changeVideo(String videoUrl, String episodeTitle) async {
    print("🎬 EpisodeScreen: _changeVideo called for URL: $videoUrl, Title: $episodeTitle"); // Added log
    try {
      setState(() {
        _isLoading = true;
        _errorMessage = null;
      ...
      // Dispose do controller atual
      _chewieController?.dispose();
      await _videoPlayerController.dispose();
      print("🎬 EpisodeScreen: Old controllers disposed."); // Added log

      // Fetch video links for the selected episode URL
      final videos = await _animeScraper.fetchVideoList(videoUrl);
      print("🎬 EpisodeScreen: Fetched ${videos.length} video links for change."); // Added log
      ...
      final selectedVideo = videos[_selectedVideoIndex];
      print("🎬 EpisodeScreen: Changing to video: ${selectedVideo.quality} - ${selectedVideo.url}"); // Added log
      ...
      await _videoPlayerController.initialize();
      print("🎬 EpisodeScreen: VideoPlayerController re-initialized. isInitialized: ${_videoPlayerController.value.isInitialized}, hasError: ${_videoPlayerController.value.hasError}, errorDescription: ${_videoPlayerController.value.errorDescription}"); // Added log
      ...
      print("🎬 EpisodeScreen: ChewieController re-initialized."); // Added log

      setState(() {
        _isInitialized = true;
        _isLoading = false;
      });
      print("🎬 EpisodeScreen: Player change complete."); // Added log

    } catch (e) {
      print("💥 EpisodeScreen: Error changing video: $e"); // Modified log
      if (!mounted) return;
      setState(() {
        _errorMessage = e.toString();
        _isLoading = false;
      ...
      main_app.scaffoldMessengerKey.currentState?.showSnackBar(
        SnackBar(content: Text('Erro ao mudar vídeo: $e'), backgroundColor: Colors.red),
      );
    }
  }

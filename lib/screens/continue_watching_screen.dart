...
class ContinueWatchingScreen extends StatelessWidget {
  ...
  Future<void> _playEpisode(BuildContext context, HistoryEntry entry) async {
    ...
    try {
      final videos = await selectedSource.fetchVideoList(entry.episode!.url as String);
      final allEpisodes = await selectedSource.fetchEpisodeList(entry.anime.url as String);
  ...
  Widget build(BuildContext context) {
    return CupertinoPageScaffold(
      navigationBar: const CupertinoNavigationBar(
        middle: Text('Continuar Assistindo'),
      ),
      child: Consumer<HistoryService>(
        builder: (context, historyService, child) {
          final history = historyService.history.value
              .where((entry) => entry.lastPosition.inSeconds > 0)
              .toList();

          if (history.isEmpty) {
            return const Center(
              child: Text('Nenhum vídeo para continuar assistindo.'),
            );
          }
              ...
              return CupertinoListTile(
                ...
                subtitle: Column(
                  ...
                  children: [
                    ...
                    Text('Parou em: ${entry.lastPosition.toString().split('.').first}'),
                  ],
                ),
